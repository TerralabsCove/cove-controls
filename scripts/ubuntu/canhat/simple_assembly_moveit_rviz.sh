...
cleanup() {
  ...
  kill "${warehouse_connect_pid:-}" 2>/dev/null || true
  wait "${warehouse_connect_pid:-}" 2>/dev/null || true
}

trap cleanup EXIT INT TERM

start_moveit_warehouse simple_assembly_moveit_config
warehouse_rviz_args=(
...
ros2 launch simple_assembly_moveit_config moveit_rviz_canhat.launch.py \
  "${warehouse_rviz_args[@]}" \
  "$@"
